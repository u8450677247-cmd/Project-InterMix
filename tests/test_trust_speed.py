...
class ProviderVaultTests(unittest.TestCase):
    def test_vault_is_private_and_values_are_never_reported(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            status = provider_vault.provider_configuration_status(path)
            self.assertTrue(status["SERPAPI_API_KEY"])
            self.assertTrue(status["TINYFISH_API_KEY"])
            self.assertNotIn("secret-test-value", repr(status))
